{
    "strategy_name": "SuperReversal_mtf",
    "params": {
      "roi": {
        "0": 500.0
      },
    ...
    "ft_stratparam_v": 1,
    "export_time": "2022-12-02 12:59:54.935445+00:00"
  }
...
class SuperReversal_mtf(IStrategy):
    ...
    st_short_atr_window = IntParameter(8, 20, default=14, space="buy", optimize=True)
    st_short_atr_multiplier = DecimalParameter(1.0, 6.0, decimals=1, default=5.1, space="buy", optimize=True)

    short_ema_window = IntParameter(3, 600, default=32, space="buy", optimize=True)
    long_ema_window = IntParameter(10, 600, default=538, space="buy", optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                    (dataframe["ema_short_val"] < dataframe["ema_long_val"])
                    &
        ...
        dataframe.loc[
            (
                    (dataframe["ema_short_val"] < dataframe["ema_long_val"])  # same as above on purpose, this is just to check that ema short periods is less than ema short periods
                    &
                    (dataframe["super_trend_direction_1h"] == -1)
                    &
                    (dataframe["ema_short_1h"] < dataframe["ema_long_1h"])
